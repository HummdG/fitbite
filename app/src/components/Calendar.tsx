...
type Props = {
  ...
  marked: Set<string>; // days that have logs
...
export function Calendar({ month, selected, marked, onSelect, onPrev, onNext }: Props) {
    ...
    <View style={styles.wrap}>
      <View style={styles.header}>
        <Pressable accessibilityRole="button" onPress={onPrev} hitSlop={8}>
          <Icon name="chevronBack" size={22} color={theme.color.purple} />
        </Pressable>
        <Text style={styles.title}>
          {MONTHS[m]} {year}
        </Text>
        <Pressable accessibilityRole="button" onPress={onNext} hitSlop={8}>
          <Icon name="chevron" size={22} color={theme.color.purple} />
        </Pressable>
      </View>
        ...
        {cells.map((day, i) => {
          ...
          return (
            <Pressable key={i} style={styles.cell} onPress={() => onSelect(k)} accessibilityRole="button">
              ...
              <View style={[styles.dot, marked.has(k) && !isSel && styles.dotOn]} />
            </Pressable>
          );
        })}
      </View>
...
const styles = StyleSheet.create({
  ...
  dayCircleToday: { borderWidth: 1, borderColor: theme.color.blushPink },
  dayText: { fontSize: theme.fontSize.body, color: theme.color.textPrimary },
  dayTextSel: { color: theme.color.textOnPink, fontWeight: '700' },
  dot: { width: 5, height: 5, borderRadius: 3, backgroundColor: 'transparent' },
  dotOn: { backgroundColor: theme.color.candyPink },
});
